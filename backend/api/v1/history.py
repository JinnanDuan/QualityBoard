...
# HistoryItem:  单条记录的响应格式；HistoryQuery: 查询参数的格式
from backend.schemas.history import HistoryItem, HistoryQuery
# list_history: Service 层的查询函数
...
async def get_history_list(
    ...
    start_time: str = Query(None),               # 筛选轮次（可选）
    case_result: str = Query(None),              # 筛选执行结果（可选）
    platform: str = Query(None),                 # 筛选平台（可选）
