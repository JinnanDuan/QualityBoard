...
| `v1/history.py` | `/api/v1/history` | 执行明细：`GET /history` 分页筛选；`POST /history/failure-process` 失败标注；`POST /history/inherit-failure-reason` 继承；`POST /history/one-click-analyze` 一键分析（整批 bug） | ✅ 已实现 |
...
| 文件 | 说明 | 实现状态 |
|------|------|---------|
| `history_service.py` | `list_history(db, query)` — 单表分步查询（禁止 JOIN）：先查 pipeline_history 主表，再按 (case_name, start_time, platform) 批量查 pipeline_failure_reason 拼装；调用 `case_dev_owner_helpers` 拼装「用例开发责任人」展示；`get_history_options()` 独立单表去重 | ✅ 已实现 |
