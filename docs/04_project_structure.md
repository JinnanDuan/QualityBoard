...
| 文件 | 说明 | 实现状态 |
|------|------|---------|
| `history_service.py` | `list_history(db, query)` — 单表分步查询（禁止 JOIN）：先查 pipeline_history 主表，再按 (case_name, start_time, platform) 批量查 pipeline_failure_reason 拼装；`get_history_options()` 独立单表去重，与列表查询解耦 | ✅ 已实现 |
