...
| 文件 | 说明 | 实现状态 |
|------|------|---------|
| `history_service.py` | `list_history(db, query)` — 单表分步查询（禁止 JOIN）：先查 pipeline_history 主表，再按 (case_name, start_time, platform) 批量查 pipeline_failure_reason 拼装；按当前页 `main_module` 批量查 `ums_module_owner`（必要时 `ums_email`）拼装「用例开发责任人」展示（姓名+工号）；`get_history_options()` 独立单表去重，与列表查询解耦 | ✅ 已实现 |
