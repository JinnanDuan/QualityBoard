...
| `history.py` | 执行明细：`HistoryItem`（单条记录响应）、`HistoryQuery`（查询参数，支持按轮次/结果/平台筛选） | ✅ 已实现 |
...
| 文件 | 说明 | 实现状态 |
|------|------|---------|
| `history_service.py` | `list_history(db, query)` — 分页查询 pipeline_history，支持动态筛选（轮次/结果/平台）+ 排序 + 分页，返回 `(items, total)` | ✅ 已实现 |
...
| 文件 | 说明 | 实现状态 |
|------|------|---------|
| `history/HistoryPage.tsx` | 详细执行历史页面。使用 Ant Design `Table` 展示 pipeline_history 数据，支持分页，执行结果以 `Tag` 颜色区分（绿色 passed / 红色 failed） | ✅ 已实现 |
