...
> 范围冻结补充（2026-04-23）：**穿刺阶段明确不做 C2/C3**。  
> 即：不交付「追问 + Session（C2）」与「一键入库 owner 全规则（C3）」；穿刺仅保障单条分析闭环可演示。
...
| C2 | **追问 + Session** | 架构 §7、§5 追问分支（**不在穿刺范围**） |
| C3 | **一键入库与「分析处理」owner 全规则** | bug 按模块 / 非 bug 按预设映射（架构 §1.4，**不在穿刺范围**） |
