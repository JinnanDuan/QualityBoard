...
**A1 不包含**：dt-report 侧 `ai_proxy` / `ai_context_builder`（**A2**，详见 `dt-report-phase-a2-ai-context-builder-spec.md`）、SSE 进度事件的丰富度与 `report` 全字段契约的终态收紧（**A3**）、接受/拒绝写库（**A4**）、按 `history_id` 限流（**A5**）、报告/截图拉取 / CodeHub 等 Tool（**Phase B**）。
...
| 单轮无 Tool | 五 Skill / Tool 矩阵属后续 | A1 **禁止**实现报告/截图拉取、CodeHub；仅基于 **请求 JSON 文本** 调单轮 LLM（或 Mock） |
| 健康检查 | §11.1 `GET /healthz` | A1：**进程可用** + 可选 `checks`；**不要求** report/screenshot / CodeHub / LLM 远程探测（可标 `skipped` / `not_configured`） |
...
A1 **不要求**对报告/截图源、CodeHub、LLM 供应商做**周期性**远程健康探测；架构 §11.1 完整 checks 可在 **B 阶段** 或运维迭代中补齐。
...
- 不实现 **httpx 拉取截图/报告**、**CodeHub**、多 Skill、Plan/Act/Synthesize、追问 session 持久化。
