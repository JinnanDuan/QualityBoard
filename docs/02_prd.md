# 项目 PRD 与系统契约 (System Context) — 最终版
...
### 1.2 业务背景与痛点
本系统服务于团队内部的自动化测试流程。当前，测试用例通过 Jenkins 流水线进行批量执行，执行结果数据已经落盘到一套固定的 MySQL 数据库中。但存在以下核心痛点：

| # | 痛点 | 影响 |
|---|------|------|
| P1 | 测试用例执行结果分散，缺乏集中展示的可视化看板 | 团队无法快速了解整体测试健康度 |
| P2 | 失败用例的归因分析、流转指派依赖线下沟通，效率低 | 问题响应周期长，容易遗漏 |
| P3 | 缺乏面向管理者的总结报告能力 | 测试进度不透明，决策缺乏数据支撑 |
...
|------|------|
| **权限定义** | 拥有**最高读写与管理**权限 |
| **允许的操作** | 包含普通用户的所有权限，额外拥有： <br> ① 管理测试用例上下线状态（`pipeline_cases.is_online`、`state` 等字段） <br> ② 管理用户账号（`ums_email` 表的增删改） <br> ③ 管理模块-责任人映射（`ums_module_owner` 表的增删改） <br> ④ 管理失败类型字典（`case_failed_type` 表的增删改） <br> ⑤ 管理下线类型字典（`case_offline_type` 表的增删改） <br> ⑥ 生成、分享批次总结报告 <br> ⑦ 配置定时催办通知规则 <br> ⑧ 手动触发紧急催办 |

### 2.2 认证方式
- 系统需实现统一账号登录（基于 `ums_email` 表中的员工信息）。
- 登录后系统需识别用户角色，动态展示/隐藏对应功能入口。
- 角色标识的存储方式：由于现有库表中无角色字段，**建议在后端通过配置文件或环境变量维护管理员工号列表**，作为角色判断依据（无需修改 DB 结构）。
...
- **实现建议：** 由于不允许新建数据库表，报告快照数据建议以 JSON 文件形式持久化到后端文件系统或对象存储中，由后端服务管理快照的读写。
...
- **存储方式：** 由于不允许新建表，审计日志建议以结构化日志文件（JSON Lines 格式）写入后端文件系统，或者使用已有表的扩展字段（如 `pipeline_history.owner_history`）进行有限的操作留痕。
...
1. **本系统底层使用已存在的 MySQL 数据库 `dt_infra`**，库表结构已完全固定。
2. **严禁 DDL 操作：** 绝对不允许生成任何 `CREATE TABLE`、`ALTER TABLE`、`DROP TABLE`、`CREATE INDEX` 语句。
3. **严禁 DML 删除操作：** 不允许对核心执行数据（`pipeline_history`、`pipeline_overview`）执行 `DELETE` 操作。
4. **逻辑实现原则：** 所有复杂的业务聚合需求，必须基于现有的表结构通过高效的 SQL `SELECT` 查询，或者在后端服务的内存中进行处理。
5. **新增持久化需求：** 如报告快照、审计日志等需要持久化但无对应表的数据，使用后端文件系统（JSON 文件等）存储，不新建表。
